package oob.instagramapitest.InstagramAPI;
...
public class CustomInstagramUploadVideoRequest extends InstagramRequest<StatusResult> {
    ...
    private MultipartBody createMultipartBody(String uploadId) throws IOException {
        return (new MultipartBody.Builder(this.api.getUuid())).setType(MultipartBody.FORM).addFormDataPart("upload_id", uploadId).addFormDataPart("_uuid", this.api.getUuid()).addFormDataPart("_csfrtoken", this.api.getOrFetchCsrf((HttpUrl) null)).addFormDataPart("media_type", "2").build();
    }
